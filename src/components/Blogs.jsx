        ...
        posts.map((post) => (
          <div key={post.id} className="mb-3 p-4 shadow-md bg-white">
            ...
            <p className="text-slate-500 text-sm">
              By <span className="font-bold">{post.author}</span> on <span className="font-bold">{post.category}</span>
            </p>
            ...
            <div className="flex gap-2 mt-3 flex-wrap">
              {post.tags.map((tag, index) => {
                return (
                <span key={index} className="text-sm text-slate-600 underline"> #{tag} </span>
                )
              })}
            </div>
          </div>
